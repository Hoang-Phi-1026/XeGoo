...
class PriceController {
    ...
    public function index() {
        ...
        $vehicleTypeId = $_GET['vehicle_type'] ?? null; // Changed from vehicleId to vehicleTypeId
        ...
        $vehicleTypes = Price::getAllVehicleTypes(); // Changed from getAllVehicles to getAllVehicleTypes
    ...
    public function store() {
        ...
        // Vehicle Type - Changed from maPhuongTien to maLoaiPhuongTien
        ...
        if (empty($errors)) {
            if (Price::priceExists($data['maTuyenDuong'], $data['maLoaiPhuongTien'], $data['loaiChoNgoi'], $data['maLoaiVe'], $data['ngayBatDau'], $data['ngayKetThuc'])) {
                $errors[] = 'Đã tồn tại cấu hình giá vé tương tự cho loại phương tiện này trong khoảng thời gian này.'; // Updated error message
            }
        }
    ...
    public function update($id) {
        ...
        if (empty($errors)) {
            if (Price::priceExists($data['maTuyenDuong'], $data['maLoaiPhuongTien'], $data['loaiChoNgoi'], $data['maLoaiVe'], $data['ngayBatDau'], $data['ngayKetThuc'], $id)) {
                $errors[] = 'Đã tồn tại cấu hình giá vé tương tự cho loại phương tiện này trong khoảng thời gian này.'; // Updated error message
            }
        }
    ...
    public function search() {
        ...
        $criteria = [
            'search' => $_GET['search'] ?? '',
            'routeId' => $_GET['route'] ?? '',
            'vehicleTypeId' => $_GET['vehicle_type'] ?? '', // Changed from vehicleId to vehicleTypeId
